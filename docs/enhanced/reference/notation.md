...
```text
    ...
    m            de-risking multiplier in [0, 1]; prefixed variants
                 ...
                 meaning (see drawdown-overlay, china-sizing-stack, meta-labeling notes)
```
...
```text
    ...
    N_eff        effective number of independent trials
                 N_eff = (sum eig)^2 / sum(eig^2) over the trial-PnL
                 correlation matrix eigenvalues (local name: eig)

    band         no-trade dead-zone threshold in z units
    mean_w/std_w rolling mean and std over window w bars
    span         EWMA smoothing span in bars; lambda_s = 2/(span+1)
    w            rolling z-score window length in bars
    beta_i       exposure of candidate PnL to pool member i (Component 3)
    epsilon(t)   residual PnL after regressing out pool (Component 3)
```
...
---
## Related notes
- [style-guide](../style-guide.md) - presentation rules for formula blocks
- [ic-metrics-and-horizon-ladder](../concepts/mining/ic-metrics-and-horizon-ladder.md) - IC family detail
