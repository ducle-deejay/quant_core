...
```text
    ...
    p(t-1)     position decided at end of bar t-1 from data <= t-1 (anti-lookahead)
    r(t)       asset return over bar t
    c          cost per unit notional per side = fee + half-spread + buffer
    abs(...)   fees are direction-blind: +5 then -5 sums to zero signed but pays on ten

    Key insight: the position change IS the order size by accounting identity;
    order-book mechanics belong to Component 6.
```
...
- Related: [canonical-mapping](canonical-mapping.md), [no-trade-band](no-trade-band.md), [harness-parameters](harness-parameters.md)
