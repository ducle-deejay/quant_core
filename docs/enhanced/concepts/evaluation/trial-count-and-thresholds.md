...
Raw N overcounts because GA output correlates heavily. Effective N comes from clustering survivors by PnL correlation (cut near 0.7) and counting clusters, or from the participation ratio of the trial-correlation eigenvalues: N_eff = (sum eig)^2 / sum(eig^2), where eig are eigenvalues
of the trial-PnL correlation matrix. Some shops skip explicit N and permutation-test each batch instead.
...
- Related: [stage-0-alpha-mining](../../stages/stage-0-alpha-mining.md), [ic-metrics-and-horizon-ladder](../mining/ic-metrics-and-horizon-ladder.md)
