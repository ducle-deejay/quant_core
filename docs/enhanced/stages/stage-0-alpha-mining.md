...
Component 0 is a factory that converts hypotheses into thousands of stateless score functions through a DSL grammar, hand-written seeds, and genetic search. Survivors are funneled, clustered into families, and handed to [stage-1-canonical-simulation](stage-1-canonical-simulation.md). This hub synthesizes several discussion exchanges; there is no frozen original file for this component. The runnable companion is `research/seed_alpha_demo.py` (see [seed-alpha-demo](../case-studies/seed-alpha-demo.md)).

Component 0 documents two signal-generation families - interpretable formulaic expressions (current mode: hand-written seeds bred by genetic search) and ML-based generators (future research direction, entry conditions recorded). Family taxonomy and expansion triggers: [alpha-generation-methods](../concepts/mining/alpha-generation-methods.md).
...
5.2 Grammar and breeding. Humans supply hypothesis seeds; machines enumerate or genetically breed millions of expressions; vectorized backtests screen tens of thousands per day on multi-component fitness (net Sharpe, decay, turnover, capacity). See [ga-machinery](../concepts/mining/ga-machinery.md).

5.3 Orthogonalization. A new alpha must show incremental value after regressing out existing pool PnL. See [stage-3-orthogonalization](stage-3-orthogonalization.md).
...
6.2 Acceptance criteria are formal gates applied later in [stage-2-evaluation-screening](stage-2-evaluation-screening.md): canonical-PnL Sharpe, walk-forward stability, parameter plateau, deflated thresholds.
...
```text
    ...
    ICIR      Sharpe ratio on the alpha's IC time series
              (see ic-metrics-and-horizon-ladder note)
...
```text
    N_eff = (sum of eig)^2 / sum of eig^2

    eig_i   eigenvalues of the trial-PnL correlation matrix
```

---
## Related notes
- [stage-1-canonical-simulation](stage-1-canonical-simulation.md) - where survivors go next
- [formulaic-alpha-and-seeds](../concepts/mining/formulaic-alpha-and-seeds.md) - definitions and grammar detail
- [ic-metrics-and-horizon-ladder](../concepts/mining/ic-metrics-and-horizon-ladder.md) - how forecast quality is measured here
- [ga-machinery](../concepts/mining/ga-machinery.md) - breeding engines and fitness design
- [seed-alpha-demo](../case-studies/seed-alpha-demo.md) - worked case study with runnable script
